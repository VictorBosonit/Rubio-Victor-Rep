...
public class ServicePerson implements  InterfaceServicePerson{
    ...
    public String loginUser(String userName , String password) throws Exception {
        ...
        List<Person> person = personRepository.findByName(userName);

        System.out.println("El nombre del usuario es: " + person.get(0).getName() + "\nLa clave del usuario es: " + person.get(0).getPassword());
        if (person == null) {
            System.out.println("Persona no encontrada");
        }
        if(userName.equals(person.get(0).getName()) && password.equals(person.get(0).getPassword())) {
            if (person.get(0).isAdmin()) {
                System.out.println("El usuario " + person.get(0).getName() + " es ADMIN");
                return getJwtToken(userName, "ROLE_ADMIN");
            } else {
                System.out.println("El usuario " + person.get(0).getName() + " es USER");
