...
public class ServicePerson implements  InterfaceServicePerson{
    ...
    @Override
    public List<Person> getAllPersonByNameSurname(String search , String parameter, String order, int numPage) throws Exception {
        ...
        if (search.equals("users")){
            predicates.add(cb.like(personRoot.get("users"),"%"+parameter+"%" ));
        }
        if (search.equals("name")){
            predicates.add(cb.like(personRoot.get("name"),"%"+parameter+"%"));
        }
        if (search.equals("surname")){
            predicates.add(cb.like(personRoot.get("surname"),"%"+parameter+"%"));
        }
        if (search.equals("date")){

            Date newDate = new SimpleDateFormat("yyyy/mm/dd").parse(parameter);

            predicates.add(cb.greaterThan(personRoot.get("created_date"), newDate));
        }
        ...
        if(order.equals("name")|| order.equals("surname")) {
            cq.orderBy( // Order by
                    cb.asc( // ascending order
                            personRoot.get(order))); // Column name
        }

        PagedListHolder<Person> pagedListHolder = new PagedListHolder<>(em.createQuery(cq).getResultList());
        pagedListHolder.setPageSize(1);
        pagedListHolder.setPage(numPage);
