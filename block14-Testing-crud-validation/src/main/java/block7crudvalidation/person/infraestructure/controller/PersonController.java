...
public class PersonController {
    ...
    public ResponseEntity getAllPerson() {
        try {
            ...
            return ResponseEntity.status(HttpStatus.NOT_FOUND).body("{\"error\":\"Error, por favor vuelva a intentar.\"}");
        }
    }

    // with @Putmaping, we can update dates in BB DD in this case we use the "id"
    @PutMapping("/{id}")
    public ResponseEntity updatePerson(@PathVariable("id") Long id,InputPersonDto personDto) throws Exception {
    ...
    //method for exercise 13
    @GetMapping("/criteria/{search}")
    public ResponseEntity<Teacher> findByDate(@PathVariable("search")String search,
                                              @RequestParam("parameter")String parameter,
                                              @RequestParam(value = "order")String order,
                                              @RequestParam ("numPage") int numPage)
                                              throws Exception {
        return new ResponseEntity(servicePerson.getAllPersonByNameSurname(search, parameter, order, numPage), HttpStatus.OK);
    }
